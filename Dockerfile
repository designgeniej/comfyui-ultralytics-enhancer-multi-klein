...
WORKDIR /comfyui
# Temporary test path: the FLUX model is hosted as a .rar, so install 7z.
RUN apt-get update && \
...
RUN mkdir -p /comfyui/models/diffusion_models /tmp/flux-rar && \
    ...
    EXTRACTOR="$(command -v 7z || command -v 7zz)" && \
    "$EXTRACTOR" x -y /tmp/flux-rar/flux.rar -o/tmp/flux-rar/extracted && \
